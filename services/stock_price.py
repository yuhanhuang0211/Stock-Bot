...
def get_stock_price(stock_id: str) -> str:
    ...
    headers = {
        ...
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36"  # 保持 User-Agent 更新
    }

    try:
        response = requests.get(url, headers=headers, timeout=10)
        response.raise_for_status()  # 檢查 HTTP 錯誤（例如 404、500）
        data = response.json()
        ...
        if data.get('msgArray') and len(data['msgArray']) > 0:
            ...
            # API 可能回傳空 msgArray 或錯誤代碼（如 '0501' 表示查無資料）
            api_error_code = data.get('rtcode')
            logging.info(f"查無股票代號 {stock_id}（TSE） 的資料。API 回應: {data}")
        ...
        return f"無法取得股票資料({stock_id})，網路連線失敗。"
        ...
        logging.exception(f"取得股票 {stock_id} 資料時發生未預期錯誤")
